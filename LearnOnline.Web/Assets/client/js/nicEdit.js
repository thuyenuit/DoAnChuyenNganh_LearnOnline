 ...
 */
function BuildNicEditor(elementId, width) {
    if (width) {
        $('#' + elementId).css('width', width);
        $('#' + elementId).css('height', '50px');
    }
    new nicEditor().panelInstance(elementId);
}
...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : '/Contents/nicEdit/nicEditorIcons.gif',
	//buttonList : ['save','bold','italic','underline','left','center','right','justify','ol','ul','fontSize','fontFamily','fontFormat','indent','outdent','image','upload','link','unlink','forecolor','bgcolor','latex','hr'],
	buttonList: ['save', 'bold', 'italic', 'underline', 'left', 'center', 'right', 'justify', 'ol', 'ul', 'fontSize', 'fontFamily', 'fontFormat', 'indent', 'outdent', 'link', 'unlink', 'forecolor', 'bgcolor', 'hr', 'upload', 'latex'],
	iconList : {"xhtml":1,"bgcolor":2,"forecolor":3,"bold":4,"center":5,"hr":6,"indent":7,"italic":8,"justify":9,"left":10,"ol":11,"outdent":12,"removeformat":13,"right":14,"save":25,"strikethrough":16,"subscript":17,"superscript":18,"ul":19,"underline":20,"image":21,"link":22,"unlink":23,"close":24,"arrow":26,"upload":27,"latex":28}
...
var nicEditorAdvancedButton = nicEditorButton.extend({
	...
	removePane : function() {
		if(this.pane) {
			this.ne.selectedInstance.setContent(this.ne.selectedInstance.getContent().replace('{latex}',''));
			this.pane.remove();
			this.pane = null;
...
/* END CONFIG */
var editorNewContent = '';
var nicLinkButton = nicEditorAdvancedButton.extend({
	addPane : function() {
	    //this.ne.selectedInstance.setContent(this.ne.selectedInstance.getContent().replace('{latex}', "\\(" + this.latexArea.value + "\\)"));

	    var oldContent = this.ne.selectedInstance.getContent();
	    this.ne.nicCommand("createlink", "nicCreatelinkUrl");
	    editorNewContent = this.ne.selectedInstance.getContent();
	    this.ne.selectedInstance.setContent(oldContent);
	...
	submit : function(e) {
		...
		this.ne.selectedInstance.setContent(editorNewContent.replace('nicCreatelinkUrl', url));
	}
});
...
var nicLatexOptions = {
	buttons : {
		'latex' : {name : 'Input latex', type : 'nicLatexButton'}
	}
	
};
/* END CONFIG */

var nicLatexButton = nicEditorAdvancedButton.extend({
  width : '640px', height:'450px',
  addPane : function() {
    var prefixId = Math.random();
	this.ne.nicCommand("insertHTML", "{latex}");
    var container = new bkElement('div')
      .setStyle({ padding: '10px' })
      .appendTo(this.pane.pane);

		new bkElement('div')
		.setAttributes({id : 'latexToolbar' + prefixId})
      .setStyle({ fontSize: '14px', fontWeight: 'bold', paddingBottom: '5px', width: '630px' })
      .appendTo(container);
	  this.latexArea=new bkElement('textarea')
		.setAttributes({id : 'latexInput' + prefixId})
		.setStyle({width: '630px' })
      .appendTo(container);
	  new bkElement('div')
      .setContent('<br />')
      .appendTo(container);
	  new bkElement('img')
		.setAttributes({id : 'equation' + prefixId})
      .appendTo(container);
	  
	  EqEditor.embed('latexToolbar' + prefixId, '', 'sty,spa,bin,sym,for,sub,acc,ace,arr|ope,bra,gel,geu,rel,mat,geo', 'vi-vi');
		EqEditor.add(new EqTextArea('equation' + prefixId, 'latexInput' + prefixId),false);
		
		new bkElement('div')
      .setContent('<br /><br />')
      .appendTo(container);
		this.saveButton = new bkElement('input')
      .setAttributes({ 'type' : 'button', 'value':'Insert' })
      .appendTo(container);
	  
	  this.saveButton.onclick = function() {
	  if(this.latexArea.value!='')
	  {
		this.ne.selectedInstance.setContent(this.ne.selectedInstance.getContent().replace('{latex}',"\\("+this.latexArea.value+"\\)"));
	  }
	  this.removePane();
...
nicEditors.registerPlugin(nicPlugin,nicLatexOptions);
...
var nicUploadButton = nicEditorAdvancedButton.extend({	
    nicURI: 'http://cdn.powergatevn.com/Upload/',
  ...
  uploadFile : function() {
    ...
    fd.append("key", "htnb7ea18a4ecbda8e92203fa4968d10660");
