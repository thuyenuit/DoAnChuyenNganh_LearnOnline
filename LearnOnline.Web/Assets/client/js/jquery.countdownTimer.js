...
(function( $ ) {
    ...
    function onlyMinutes($this, opts) {
        ...
        if(window['minutes_M'+ id] == opts.minutes && window['seconds_M'+ id] == opts.seconds) {
			...
			//alert(window['minutes_M'+ id]);
			if(window['minutes_M'+ id] < 10) {
			//alert(window['minutes_M'+ id]);
                runningOutOfTime($this, opts);
            }
            if(window['minutes_M'+ id]==0 && window['seconds_M'+ id] < 0)
            {
                delete window['hours_M'+ id];
    ...
    function currentDate($this, opts) {
        if (window['currentTime' + $this.attr('id')] == true) {
            ...
            var today = new Date();
            var offset = (today.getTime() - startDate.getTime()) / 1000;

            var hours = parseInt(offset / 60 / 60);
	...
	function runningOutOfTime($this, opts) {
		var alertStyle = {
                "background": '#FFFB00',
                "border-color": '#FFFB00',
                "color":"#FA0909"
		...
		if (!playedAlert) {
		    ...
		    audioElement.setAttribute('src', '/Contents/sound/examAlert.mp3');
